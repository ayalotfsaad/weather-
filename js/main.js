...
async function getApi(cityName) {
    let url = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=9e217eea803c4c058ae215425240401&q=${cityName}&days=3&aqi=no&alerts=no`)
...
function display(data) {
    ...
    forcastIcone.setAttribute("src", "https:" + data.current.condition.icon)
...
function displayNextday(data) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        nextDayIcon[i].setAttribute("src", "https:" + forcasetdata[i + 1].day.condition.icon)
    }
}
